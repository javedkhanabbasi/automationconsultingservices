...
export default async function AdminDashboard() {
  ...
  const stats = [
    { label: 'Blog posts', count: postsCount.count || 0, href: '/acs-1000-admin/blog', cta: '+ New post' },
    { label: 'Case studies', count: csCount.count || 0, href: '/acs-1000-admin/case-studies', cta: '+ New case study' },
    { label: 'Services', count: servicesCount.count || 0, href: '/acs-1000-admin/services', cta: 'Manage' },
    { label: 'Pages', count: pagesCount.count || 0, href: '/acs-1000-admin/pages', cta: 'Manage' },
  ];
    ...
    <div className="p-4 sm:p-8 lg:p-12 max-w-7xl">
      ...
      <div className="mb-8 sm:mb-12">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <Link href="/acs-1000-admin/blog/new" className="bg-lime text-black rounded-xl p-4 sm:p-5 hover:bg-lime-dark transition-colors block">
            <div className="text-2xl mb-2">✎</div>
            <div className="font-bold text-sm">Write blog post</div>
          </Link>
          <Link href="/acs-1000-admin/case-studies/new" className="bg-black text-white rounded-xl p-4 sm:p-5 hover:bg-ink-90 transition-colors block">
            <div className="text-2xl mb-2">★</div>
            <div className="font-bold text-sm">New case study</div>
          </Link>
          <Link href="/acs-1000-admin/media" className="card p-4 sm:p-5 hover:border-black transition-colors block">
            <div className="text-2xl mb-2">📁</div>
            <div className="font-bold text-sm text-black">Upload media</div>
          </Link>
          <Link href="/acs-1000-admin/categories" className="card p-4 sm:p-5 hover:border-black transition-colors block">
            <div className="text-2xl mb-2">⊞</div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="card p-4 sm:p-6">
          <div className="flex items-center justify-between mb-4 sm:mb-5">
            ...
            <Link href="/acs-1000-admin/blog" className="text-sm font-semibold text-black hover:text-lime-dark">View all →</Link>
          </div>
          {recentPosts.data && recentPosts.data.length > 0 ? (
            <ul className="space-y-2 sm:space-y-3">
              {recentPosts.data.map((p) => (
                <li key={p.id}>
                  <Link href={`/acs-1000-admin/blog/${p.id}`} className="flex items-center justify-between p-2 sm:p-3 rounded-md hover:bg-ink-5 transition-colors gap-2">
            ...
            <p className="text-sm text-ink-50 text-center py-8">No posts yet. <Link href="/acs-1000-admin/blog/new" className="text-black font-semibold underline">Write the first one</Link>.</p>
          )}
        </div>
        ...
        <div className="card p-4 sm:p-6">
          <div className="flex items-center justify-between mb-4 sm:mb-5">
            ...
            <Link href="/acs-1000-admin/case-studies" className="text-sm font-semibold text-black hover:text-lime-dark">View all →</Link>
          </div>
          {recentCs.data && recentCs.data.length > 0 ? (
            <ul className="space-y-2 sm:space-y-3">
              {recentCs.data.map((c) => (
                <li key={c.id}>
                  <Link href={`/acs-1000-admin/case-studies/${c.id}`} className="flex items-center justify-between p-2 sm:p-3 rounded-md hover:bg-ink-5 transition-colors gap-2">
                    <div className="flex-1 min-w-0">
