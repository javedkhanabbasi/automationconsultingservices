  ...
  return (
    <div className="p-8 lg:p-12 max-w-7xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
        {stats.map((s) => (
          <Link key={s.label} href={s.href} className="card p-6 hover:border-black transition-colors group">
            <div className="text-xs font-bold text-ink-60 uppercase tracking-wider mb-2">{s.label}</div>
            <div className="text-4xl font-bold text-black mb-3">{s.count}</div>
      ...
      {/* QUICK ACTIONS */}
      <div className="mb-12">
        <h2 className="text-xl font-bold text-black mb-5">Quick actions</h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <Link href="/admin/blog/new" className="bg-lime text-black rounded-xl p-5 hover:bg-lime-dark transition-colors block">
            <div className="text-2xl mb-2">✎</div>
            <div className="font-bold text-sm">Write blog post</div>
          </Link>
          <Link href="/admin/case-studies/new" className="bg-black text-white rounded-xl p-5 hover:bg-ink-90 transition-colors block">
            <div className="text-2xl mb-2">★</div>
            <div className="font-bold text-sm">New case study</div>
          </Link>
          <Link href="/admin/media" className="card p-5 hover:border-black transition-colors block">
            <div className="text-2xl mb-2">📁</div>
            <div className="font-bold text-sm text-black">Upload media</div>
          </Link>
          <Link href="/admin/categories" className="card p-5 hover:border-black transition-colors block">
            <div className="text-2xl mb-2">⊞</div>
          ...
          {recentPosts.data && recentPosts.data.length > 0 ? (
            <ul className="space-y-3">
              {recentPosts.data.map((p) => (
                <li key={p.id}>
                  <Link href={`/admin/blog/${p.id}`} className="flex items-center justify-between p-3 rounded-md hover:bg-ink-5 transition-colors">
        ...
        <div className="card p-6">
          ...
          {recentCs.data && recentCs.data.length > 0 ? (
            <ul className="space-y-3">
              {recentCs.data.map((c) => (
                <li key={c.id}>
                  <Link href={`/admin/case-studies/${c.id}`} className="flex items-center justify-between p-3 rounded-md hover:bg-ink-5 transition-colors">
                    <div className="flex-1 min-w-0">
